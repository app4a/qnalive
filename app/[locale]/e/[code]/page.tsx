    ...
    const fetchEvent = async () => {
      try {
        ...
        toast({
          variant: 'destructive',
          title: tc('error'),
          description: t('errors.loadEvent'),
        })
        setLoading(false)
      }
    }
                  ...
                  <SelectContent>
                    <SelectItem value="latest">
                      <div className="flex items-center gap-2">
                        ...
                        <span>{t('polls.sortLatest')}</span>
                      </div>
                    </SelectItem>
                    <SelectItem value="votes">
                      <div className="flex items-center gap-2">
                        <TrendingUp className="h-4 w-4" />
                        <span>{t('polls.sortVotes')}</span>
                      </div>
                    </SelectItem>
                  </SelectContent>
                </Select>
              ...
              return sortedPolls.length === 0 && event?.settings?.allowParticipantPolls ? (
                <Card>
                  <CardContent className="py-8 text-center">
                    ...
                    <p className="text-gray-600 mb-2">{t('polls.noPolls')}</p>
                    {currentUserId && (
                      <p className="text-sm text-gray-500">
                        {t('polls.createPollHint')}
                      </p>
                    )}
                  </CardContent>
                </Card>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 text-sm text-gray-600 mb-1">
                          <span>{t('polls.by')} {creatorName}</span>
                          <span>•</span>
                          <span>{formatDateTime(poll.createdAt)}</span>
                        </div>
                        <CardDescription>
                          {isPending 
                            ? t('polls.pendingApproval')
                            : userVotedOptionId 
                              ? t('polls.clickToRemove') 
                              : showResults 
                                ? t('polls.selectToVote')
                                : t('polls.voteToSee')}
                        </CardDescription>
                      </div>
                      {isOwnPoll && (
                          ...
                          onClick={() => {
                            if (confirm(t('polls.confirmDelete'))) {
                              fetch(`/api/events/${event.id}/polls/${poll.id}`, {
                                ...
                                toast({
                                  title: tc('success'),
                                  description: t('polls.pollDeleted'),
                                })
                              }).catch(() => {
                                toast({
                                  variant: 'destructive',
                                  title: tc('error'),
                                  description: t('errors.deletePoll'),
                                })
                              })
                            }
                          }}
                  ...
                  <CardContent>
                    ...
                    {isPending && (
                      <p className="text-xs text-center text-yellow-700 mt-4 bg-yellow-100 p-2 rounded">
                        {t('polls.awaitingApproval')}
                      </p>
                    )}
                    {!isPending && !showResults && (
                      <p className="text-xs text-center text-gray-500 mt-4">
                        {t('polls.resultsAfterVote')}
                      </p>
                    )}
                  </CardContent>
                </Card>
            ...
            return (
              <>
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-xl font-bold flex items-center">
                    <MessageSquare className="h-5 w-5 mr-2" />
                    {t('questionsTitle', { count: filteredQuestions.length })}
                  </h2>
